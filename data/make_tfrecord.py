if __name__ == '__main__':
    ...
    from tqdm import tqdm  # Package for making command line progress bars for iterable processes
    ...
    audio_fps = glob.glob(os.path.join(args.in_dir, '*.{}'.format(args.ext)))  # Get (list?) of all files in in_dir
    random.shuffle(audio_fps)  # Shuffle items in list of files

    if args.nshards > 1:
        npershard = int(len(audio_fps) // (args.nshards - 1))  # num of input files to go into each TFRecord file
    else:
        npershard = len(audio_fps)  # i.e. number of files in in_dir
    ...
    for i, start_idx in tqdm(enumerate(range(0, len(audio_fps), npershard))):  # range between 0, #files in in_dir, step every npershard
        shard_name = '{}-{}-of-{}.tfrecord'.format(args.name, str(i).zfill(len(str(args.nshards))), args.nshards)  # Generate filename for current TFRecord file
        shard_fp = os.path.join(args.out_dir, shard_name)  # full file path to current TFRecord file

        writer = tf.python_io.TFRecordWriter(shard_fp)  # Open writer to file path of current TFR

        for _audio_fp in audio_fps[start_idx:start_idx + npershard]:  # iterate through 'npershard' files to go into current TFRecord file
            audio_name = os.path.splitext(os.path.split(_audio_fp)[1])[0]
            if args.labels:  # default False
            ...
            try:
                _slices = sess.run(slices, {audio_fp: _audio_fp})  # run session to generate slices for current file?
            except:
                continue
