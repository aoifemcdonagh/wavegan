...
def train(fps, args):
    ...
    with tf.train.MonitoredTrainingSession(
        ...
        while True:
            # Train discriminator
            for i in xrange(args.wavegan_disc_nupdates):  # D updates 'nupdates' times for every 1 time G updates
                sess.run(D_train_op)
...
if __name__ == '__main__':
    ...
    if split is not None:
        fps = glob.glob(os.path.join(args.data_dir, split) + '*.tfrecord')  # Finds all tf record files in args.data_dir
